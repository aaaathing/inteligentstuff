...
for i in range(10):
		...
		# Backpropagation
		goalProposer.zero_grad()
		rewardPredictor.zero_grad()
		(actor_loss + critic_loss).backward()
		
		with torch.no_grad():
			# Update weights
			for param in goalProposer.parameters():
					param.data -= 0.01 * param.grad.data
			for param in rewardPredictor.parameters():
					param.data -= 0.01 * param.grad.data
